...
export const HeartPage = (props) => {
  ...
  const [ECGButton, setECGButtonValue] = useState();
  const [RUSBButton, setRUSBButtonValue] = useState();
  const [LUSBButton, setLUSBButtonValue] = useState();
  const [LLSBButton, setLLSBButtonValue] = useState();
  const [APEXButton, setAPEXButtonValue] = useState();

  const handleECGButtonChange = (event) => {
    setECGButtonValue(event.target.value)
  }
  const handleRUSBButtonChange = (event) => {
    setRUSBButtonValue(event.target.value)
  }
  const handleLUSBButtonChange = (event) => {
    setLUSBButtonValue(event.target.value)
  }
  const handleLLSBButtonChange = (event) => {
    setLLSBButtonValue(event.target.value)
  }
  const handleAPEXButtonChange = (event) => {
    setAPEXButtonValue(event.target.value)
  }
  const handleSave = (e) => {
    e.preventDefault();
    const data = {}
    data['ECGButton'] = ECGButton;
    data['RUSBButton'] = RUSBButton;
    data['LUSBButton'] = LUSBButton;
    data['LLSBButton'] = LLSBButton;
    data['APEXButton'] = APEXButton;
    console.log(data);

    axios({
     method:"POST",
     url: props.proxy + "/upload_json",
     data: {data: data, filename: '/heart'},
     headers: {
        ...
        <div className="overlap">
          <div className="overlap-group">
            ...
            <span className="tick-circle">
              <div>
                <CheckandXButtons />
                <FormControl value = {ECGButton} onChange={handleECGButtonChange}></FormControl>
              </div>            
            </span>
