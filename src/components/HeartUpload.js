...
//   }, [props.audio, props.audiobell]);
  ...
  useEffect(() => {
    const newHowlInstances = [];
    if (props.audio) {
      const howl = new Howl({
        src: [props.audio],
        autoplay: false,
        loop: false,
        volume: 1, // Set the volume to 1 (maximum)
      });
      newHowlInstances.push(howl);
    }
    if (props.audiobell) {
      const howlBell = new Howl({
        src: [props.audiobell],
        autoplay: false,
        loop: false,
        volume: 1, // Set the volume to 1 (maximum)
      });
      newHowlInstances.push(howlBell);
    }
  ...
  return (
    <div>
      <Popup trigger={props.audio && props.audiobell ? <Button
        aria-describedby={id}
        variant="contained"
        color="inherit"
        onClick={handleClick}
        ...
        <div className={classname}>
          ...
          {howlInstances[0] && (
            <Button onClick={() => howlInstances[0].play()}>Play Audio</Button>
          )}
          <input
            ref={fileInputRef2}
            type="file"
          ...
          {howlInstances[1] && (
            <Button onClick={() => howlInstances[1].play()}>Play Audio</Button>
          )}
        </div>
      </Popup>
    </div>
